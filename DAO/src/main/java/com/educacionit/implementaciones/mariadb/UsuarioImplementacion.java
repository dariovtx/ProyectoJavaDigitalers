...
public class UsuarioImplementacion  implements DAO< Usuario, String >, ConexionMariaDB{
	...
	public Usuario buscar(String correo) {
		...
		String query = "select correo, cast( aes_decrypt(clave, ?) as char(100)) as clave , activo from usuarios where correo = ?";
		try {
			if (null == psBuscar) {
	...
	public List<Usuario> listar() {
		...
		String query = "select correo, cast( aes_decrypt(clave, ?) as char(100)) as clave, activo from usuarios";
		try {
			if (null == psListar) {
